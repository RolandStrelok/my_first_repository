...
output_csv_filename = 'ocr_simplified_multi_area_results_test.csv' # Новое имя файла
...
def is_bbox_overlap(bbox1, bbox2):
    ...
    # Если один прямоугольник находится справа от другого ИЛИ
...
try:
    ...
    for i, page_image in enumerate(pages_to_iterate):
        ...
        print(f"   -> Найдено {len(area1_unique_sorted)} слов (с цифрами) в Области 1.")
        print(f"   -> Найдено {len(area2_unique_sorted)} слов (с цифрами) в Области 2.")
...
if recognized_data_per_page:
    ...
    for page_num in sorted_page_nums:
        ...
        print(f"Область 1 (слова с цифрами): {display_area1_words}{'...' if len(page_data_dict['area1_words']) > display_words_limit else ''}")
        ...
        print(f"Область 2 (слова с цифрами): {display_area2_words}{'...' if len(page_data_dict['area2_words']) > display_words_limit else ''}")
